...
const forgetPassword = async (req, res, { userModel }) => {
  ...
  const easy_app_email = settings['easy_app_email'];
  const easy_base_url = settings['easy_base_url'];

  const url = checkAndCorrectURL(easy_base_url);
  ...
  await sendMail({
    ...
    name: user.name,
    link,
    subject: 'Reset your password | easy',
    easy_app_email,
