...
export default function ReadItem({ config }) {
  ...
  const { readColumns, fields } = config;
  ...
  const resolvedReadColumns = useMemo(() => {
    if (fields) return dataForRead({ fields, translate });
    return readColumns || [];
  }, [fields, readColumns, translate]);

  useEffect(() => {
    if (!currentResult || !Array.isArray(resolvedReadColumns)) {
      setListState([]);
      return;
    }

    const list = [];
    resolvedReadColumns.forEach((props) => {
    ...
    setListState(list);
  }, [currentResult, dateFormat, resolvedReadColumns]);
